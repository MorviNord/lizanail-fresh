// islands/FloatingNav.tsx
//
// Плавающий нав-бар со скрытием по направлению скролла:
// - скроллим вниз -> нав уезжает вверх за экран
// - скроллим вверх (даже немного) -> нав тут же появляется
// - у самого верха страницы нав всегда виден
//
// Отличия от предыдущей версии:
// - `sticky` вместо `fixed`: элемент сам резервирует своё место в потоке,
//   Header больше не нуждается в ручном spacer-блоке с магическим числом.
// - Сырой scrollY берётся из общего хука useScrollY (throttled через rAF,
//   переиспользуется и в ScrollToTop).
// - `scrolled` — производное значение через computed(), а не отдельный
//   сигнал, который пришлось бы вручную держать в синхроне.
// - Направление скролла — единственное, что действительно требует памяти
//   о предыдущем кадре, поэтому это by design обычный useEffect с
//   preact-сигнальным effect() внутри, а не computed.
...
export default function FloatingNav({
  ...
  return (
    <div
      class={`fixed w-full top-0 z-50 flex justify-center px-4 pt-3 transition-transform duration-300 sm:px-6 ${
